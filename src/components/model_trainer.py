...
class ModelTrainer:
    ...
    def initiate_model_trainer(self, train_array, test_array) -> float:
        """
        Trains multiple regression models and selects the best one based on R2 score.

        Args:
            train_array (np.ndarray): Training data with features + target as last column
            test_array (np.ndarray): Test data with features + target as last column
